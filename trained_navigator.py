...
checkpoint = torch.load("navigator.pth", map_location="cpu", weights_only=False)
...
def predict_action(raw_features):
    ...
    if x.shape[1] != in_features:
        raise ValueError(f"Input feature length mismatch: expected {in_features}, got {x.shape[1]}")
...
ai.start(AI_loop, ["-name", "trained_navigator", "-join", "localhost"])
