...
DEFAULT_GENERATION_INTERVAL = 10 * 60
POPULATION_FILENAME = "population_GA.txt"
ARCHIVE_FILENAME = "fitness_results.txt"
UPDATES_FILENAME = "fitness_updates.jsonl"
EPS = 1e-9
RUNS_PER_CHROM = 1
ELITE_COUNT = 1
...
def load_population(fname=POPULATION_FILENAME):
    if not os.path.exists(fname):
        pop = generate_population(DEFAULT_POPULATION_SIZE)
        save_population(pop, fname)
        return pop
...
previous_score = 0.0
shots_fired = 0
_accum_score_gain = 0.0
last_active_chromo_idx = None
...
def compute_fitness_for_life(life_duration, score_gain):
    if score_gain > 0:
...
def AI_loop():
    ...
    global previous_score, shots_fired, fitness_list, eval_counts, sum_fitness, _accum_score_gain, last_active_chromo_idx
    ...
    if alive and current_life_start_time is None:
        if last_active_chromo_idx is not None:
            delayed_score_gain = score - previous_score
            if delayed_score_gain > EPS:

                delayed_fitness = compute_fitness_for_life(0, delayed_score_gain)

                sum_fitness[last_active_chromo_idx] += delayed_fitness
                if eval_counts[last_active_chromo_idx] > 0:
                    avg_so_far = sum_fitness[last_active_chromo_idx] / eval_counts[last_active_chromo_idx]
                    fitness_list[last_active_chromo_idx] = avg_so_far + EPS

                print(
                    f"[AI] **RETROACTIVE FIX** Chromo {last_active_chromo_idx} received delayed fitness +{delayed_fitness:.2f}")

                if eval_counts[last_active_chromo_idx] >= RUNS_PER_CHROM:
                    write_update_for_chromosome(last_active_chromo_idx)
        ...
        previous_score = float(ai.selfScore())
        _accum_score_gain = 0.0
    ...
    if current_chromosome is not None and alive:
        ...
        curr_score = float(ai.selfScore())
        delta = curr_score - previous_score
        if delta > 0:
            _accum_score_gain += delta
    ...
    if (not alive) and (current_life_start_time is not None) and (current_chromosome is not None):
        ...
        final_score_at_death = float(ai.selfScore())
        delta_at_death = final_score_at_death - previous_score
        if delta_at_death > 0:
            _accum_score_gain += delta_at_death


        score_gain = _accum_score_gain

        fitness = compute_fitness_for_life(life_duration, score_gain)
        ...
        print("=== LIFE ENDED ===")
        print(f"[AI] chromosome {current_chromo_idx} life_duration={life_duration:.2f}s "
              f"accum_score_gain={score_gain} last_score={final_score_at_death} fitness+={fitness:.2f}")
        print(f"[AI] chromosome {current_chromo_idx} accumulated fitness={sum_fitness[current_chromo_idx]:.2f} "
              f"runs={eval_counts[current_chromo_idx]}/{RUNS_PER_CHROM}")

        if eval_counts[current_chromo_idx] >= RUNS_PER_CHROM:
            write_update_for_chromosome(current_chromo_idx)

        last_active_chromo_idx = current_chromo_idx
        current_life_start_time = None
        current_chromosome = None
        current_chromo_idx = None
        shots_fired = 0
