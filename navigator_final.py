...
gold = [280, 7, 145, 108, 20, 57, 84, 87, 112, 31, 15]
...
#LOG_FILE = init_logger("Shooter", HEADER)
...
def norm_shot_alert(x):
    # Danger score: 1 at 80, 0 at 200, linear in between, clamped
    if x is None:
        return 0
    if x <= 80:
        return 1
...
ai.start(AI_loop, ["-name", "Navigator", "-join", "localhost"])
